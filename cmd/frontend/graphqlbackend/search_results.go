...
func (r *searchResolver) toSearchInputs(q query.Q) (*search.TextParameters, []streaming.Job, error) {
...
func (r *searchResolver) evaluateLeaf(ctx context.Context, args *search.TextParameters, jobs []streaming.Job) (_ *SearchResults, err error) {
...
func (r *searchResolver) resultsWithTimeoutSuggestion(ctx context.Context, args *search.TextParameters, jobs []streaming.Job) (*SearchResults, error) {
	start := time.Now()
...
func (r *searchResolver) doResults(ctx context.Context, args *search.TextParameters, jobs []streaming.Job) (res *SearchResults, err error) {
	...
	for _, j := range jobs {
		...
		goroutine.Go(func() {
			defer wg.Done()
			_ = agg.DoSearch(ctx, j, args.Mode)
		})
	}
