...
export const CloudSignUpPage: React.FunctionComponent<Props> = ({
    ...
    const title = sourceIsValid
        ? SourceToTitleMap[source as CloudSignUpSource]
        : isSignupOptimised
        ? SourceToTitleMap.OptimisedContext
        : SourceToTitleMap.Context // Use Context as default

    const logEvent = (type: 'github' | 'gitlab' | 'form'): void => {
        if (sourceIsValid) {
