...
func (r *queryResolver) Implementations(ctx context.Context, line, character int, limit int, rawCursor string) (_ []AdjustedLocation, _ string, err error) {
	...
	if cursor.Phase == "local" {
		localLocations, hasMore, err := r.pageLocalReferences(ctx, "implementations", adjustedUploads, &cursor, limit-len(locations))
		if err != nil {
			return nil, "", err
		}
	...
	if cursor.Phase == "remote" {
		for len(locations) < limit {
			remoteLocations, hasMore, err := r.pageRemoteReferences(ctx, "definitions", adjustedUploads, cursor.OrderedMonikers, definitionUploadIDs, &cursor, limit-len(locations))
			if err != nil {
				return nil, "", err
			}
